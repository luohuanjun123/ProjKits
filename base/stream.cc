...
StreamResult MemoryStreamBase::Write(const void* buffer, size_t bytes,
  ...
  if (0 == available) {
    ...
    size_t new_buffer_length =
        std::max<int>(((seek_position_ + bytes) | 0xFF) + 1, buffer_length_ * 2);
...
bool FifoBuffer::SetCapacity(size_t size) {
  ...
  if (size != buffer_length_) {
    ...
    const size_t tail_copy = std::min<size_t>(copy, buffer_length_ - read_position_);
...
StreamResult FifoBuffer::ReadOffsetLocked(void* buffer,
  ...
  const size_t copy = std::min<size_t>(bytes, available);
  const size_t tail_copy = std::min<size_t>(copy, buffer_length_ - read_position);
...
StreamResult FifoBuffer::WriteOffsetLocked(const void* buffer,
  ...
  const size_t write_position = (read_position_ + data_length_ + offset)
      % buffer_length_;
  const size_t copy = std::min<size_t>(bytes, available);
  const size_t tail_copy = std::min<size_t>(copy, buffer_length_ - write_position);
...
StreamResult StringStream::Read(void* buffer, size_t buffer_len,
                                      size_t* read, int* error) {
	size_t available = std::min<size_t>(buffer_len, str_.size() - read_pos_);
  if (!available)
    return SR_EOS;
